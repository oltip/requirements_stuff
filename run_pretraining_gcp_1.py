...
def main():
    ...
    parser.add_argument('--num_eval_steps', default=1000)        #default=1000)
    parser.add_argument('--num_save_steps', default=1000)     # default=2000)
...
#This piece is totally uncrlear to me, and needs to be understood. ==> Maybe listen to Abishek 
#- Not clear however, why it is different from the copy I have in my pc 
...
#        optimizer.load_state_dict(torch.load(args.model_path + "/optimizer.bin"))
    ...
    for epoch in range(args.epochs):
        ...
        print('end epoch')
        logger.info("end epoch ... now let see the results")
        now = time.time()
        eta = now - start_time
        if eta > 3600:
